...
// 							std::cout << "adding state with name=" << name << ", pointing to " << state << std::endl;
				...
				while (child != NULL && child->ValueTStr() == "TRANSITION") {
					...
					std::cerr << "adding transition from " << from_name << " with symbol " << symbol << " to " << to_name << "\n";
...
// 					State* to = states[to_name];
		...
		for (auto i = temp_transitions.begin(); i != temp_transitions.end(); ++i) {			
			...
			if (from == nullptr) {
				std::cerr << "Couldn't find valid state with name " << i->first << ", skipping...\n";
				continue;
			}
			...
			for (auto j = i->second.begin(); j != i->second.end(); ++j) {
				...
				for (int k=0; k<j->second.size(); ++k) {
					...
					std::cerr << "adding transition from " << from->name << " with symbol " << symbol << " to " << to_name << "\n"; 
				}
			}
		}
...
// 		std::cout << "the end?\n";
	...
	void Automaton::toDot(std::ostream& out) {
		...
		// lazy evaluation
		std::string start_name = start->name;
			
		state_to_dot(start_name, *this, out);
...
void state_to_dot(std::string name, Automaton& FSM, std::ostream& out) {
	std::cerr << "state_to_dot: called with " << name << "\n";
	
	State* state = FSM.states[name];
	
	if (std::find(FSM.been_there.begin(), FSM.been_there.end(), state) != FSM.been_there.end()) {
		//out << "uhuh.\n";
		return;
	} else {
		FSM.been_there.push_back(state);  // and continue
	}
	
	for (auto symbol: FSM.alphabet) {
		// if symbol is a valid transition
		if (FSM.transitions[name].find(symbol) != FSM.transitions[name].end()) {
			out << "    \"" << name << "\" -> \"" << FSM.transitions[name][symbol] 
			    << "\" [label=\"" << symbol << "\"];\n";
			state_to_dot(FSM.transitions[name][symbol], FSM, out);
		} else {
			out << "    \"" << name << "\" -> \"error"
			    << "\" [label=\"" << symbol << "\"];\n";
		}
